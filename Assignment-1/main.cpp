...
int main(int argc, char **argv) {
	...
	Mat enhanced = equalizeHistogram(original_image);
    imshow("Enhanced Image",enhanced);
	Mat nyc_image=imread("NYC_width_4times.bmp",IMREAD_GRAYSCALE);
	// generateHistogram(nyc_image,original_img_hist);
	// generateHistogram(nyc_image,original_img_hist);
	enhanced=equalizeHistogram(nyc_image);
	imshow("Enhanced Image-New York",enhanced);
	cv::waitKey(0);
	destroyAllWindows();
}